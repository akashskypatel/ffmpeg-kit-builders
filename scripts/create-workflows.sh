    ...
    steps:
      ...
      - name: Build ${platform} dependencies
        ...
        run: |
          ...
          platform="${platform}"
          arch="\${{ matrix.arch }}"
          WORKFLOW_BUILD_STEPS="\$(sudo ./runner.sh --host="\$platform" --arch="\$arch" --enable-full --gpl -y --no-bundle --skip --print-all-steps | awk -F= '/^WORKFLOW_BUILD_STEPS=/{print \$2}')"
          echo "WORKFLOW_BUILD_STEPS: \$WORKFLOW_BUILD_STEPS"

          for build in \$WORKFLOW_BUILD_STEPS; do
            echo "Running \$build for \${platform}-\${arch}"
            sudo ./runner.sh --host="\$platform" --arch="\$arch" --enable-full --gpl -y --no-bundle --skip --workflow --build-only="\$build"

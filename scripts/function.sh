...
download_file() {
  ...
  mkdir -p "${FFMPEG_KIT_TMPDIR}" 1>>"${BASEDIR}"/build.log 2>&1
  ...
  if [ ${CURL_RC} -ne 0 ]; then
    ...
    rm -f "${FILE_PATH}" 1>>"${BASEDIR}"/build.log 2>&1
    echo ${CURL_RC}
    return
  fi

  # Create library directory
  mkdir -p "${LIB_LOCAL_PATH}" 1>>"${BASEDIR}"/build.log 2>&1

...
build_alsa() {
  ...
  echo "INFO: Not available on wasm build" >>"$LOG_FILE"
}
build_libusb() {
...
-DSDL2_DIR=\"${src_dir}/sdl2\" \
-DSDL2_LIBRARY=\"${dependency_install_prefix}/lib/libSDL2.a\" \
-DSDL2_INCLUDE_DIR=\"${dependency_install_prefix}/include/SDL2\" \
...
build_bzlib() {
  ...
  disable_nonessential "$src_dir/$lib"
  generic_make_install "CC=\"${CC}\" AR=\"${AR}\" RANLIB=\"${RANLIB}\" CFLAGS=\"${CFLAGS}\""
  change_dir "$src_dir"
...
-DFontconfig_LIBRARY=${dependency_install_prefix}/lib/libfontconfig.a \
-DFontconfig_INCLUDE_DIR=${dependency_install_prefix}/include \
...
build_libcodec2() {
    ...
    local host_build="$src_dir/$lib/build_native"
    local generate_codebook="$host_build/generate_codebook"
    mkdir -p "$host_build"
    native_cross_vars
    "$CC" -O2 \
        "$src_dir/$lib/src/generate_codebook.c" \
        -o "$generate_codebook" \
...
build_libdavs2() {
  ...
  touch "no.autoreconf"
  local configure_file="$src_dir/$lib/build/linux/configure"
  local makefile="$src_dir/$lib/build/linux/Makefile"
  if ! grep -q 'emscripten\*)' "$configure_file"; then
    gsed -i '/^[[:space:]]*\*linux\*)/i\
    emscripten*)\
        SYS="EMSCRIPTEN"\
        libm="-lm"\
        ;;' "$configure_file"
  ...
  gsed -i \
    ...
    "$configure_file"
  gsed -i \
    's|$(AR)$@ $(OBJS) $(OBJAVX) $(OBJASM)|$(AR) rc $@ $(OBJS) $(OBJAVX) $(OBJASM)|' \
    "$makefile"
  gsed -i \
    's/git rev-list --count origin\/master/git rev-list --count HEAD/' \
    "$src_dir/$lib/version.sh"
  ...
  export LDFLAGS="$LDFLAGS -pthread"
  do_configure "--host=wasm32-unknown-emscripten \
--disable-cli \
--disable-asm \
--disable-opencl \
--prefix=$dependency_install_prefix"
  ...
  if [[ ! -f "$install_pkgconfig_dir/davs2.pc" && \
        -f "$src_dir/$lib/build/linux/davs2.pc" ]]; then
    copy_path \
      "$src_dir/$lib/build/linux/davs2.pc" \
      "$install_pkgconfig_dir/davs2.pc" \
      "-f"
  fi
  gsed -i \
    "s/Version:.*/Version: ${repo_ver}.0/g" \
    "$install_pkgconfig_dir/davs2.pc"
  change_dir "$src_dir"
}
...
build_libdvdcss() {
  ...
  change_dir "$src_dir/$lib"
  local meson_options=" -Denable_docs=false -Denable_examples=false"
  apply_patch "$PATCHDIR/dvdcss-1.5.0-emscripten-ioctl.patch" "$src_dir/$lib"
  apply_patch "$PATCHDIR/dvdcss-1.5.0-emscripten-dvd_region.patch" "$src_dir/$lib"
  generic_meson "$meson_options"
  disable_nonessential "$src_dir/$lib"
  do_ninja_and_ninja_install
  change_dir "$src_dir"
...
build_libflite() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  get_config_sub "$src_dir/$lib" > /dev/null 2>&1
  get_config_guess "$src_dir/$lib" > /dev/null 2>&1
...
build_libjack() {
  ...
  if [[ -f "$dependency_install_prefix/lib/libjack.so.0.1.0" ]]; then
    ...
    copy_path "$dependency_install_prefix/lib/libjack.so" "$dependency_install_prefix/lib/libjack.so.0" -f
    copy_path "$dependency_install_prefix/lib/libjack.so" "$dependency_install_prefix/lib/libjack.so.0.1" -f
    copy_path "$dependency_install_prefix/lib/libjack.so" "$dependency_install_prefix/lib/libjack.so.0.1.0" -f
  fi
  if [[ -f "$dependency_install_prefix/lib/libjackserver.so.0.1.0" ]]; then
    ...
    copy_path "$dependency_install_prefix/lib/libjackserver.so" "$dependency_install_prefix/lib/libjackserver.so.0" -f
    copy_path "$dependency_install_prefix/lib/libjackserver.so" "$dependency_install_prefix/lib/libjackserver.so.0.1" -f
    copy_path "$dependency_install_prefix/lib/libjackserver.so" "$dependency_install_prefix/lib/libjackserver.so.0.1.0" -f
  fi
  change_dir "$src_dir"
}
...
-DCMAKE_INSTALL_DATAROOTDIR=$dependency_install_prefix" "$src_dir/$lib"
  disable_nonessential "$src_dir/$lib"
  do_make_and_make_install
...
build_libmodplug() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib"
  change_dir "$src_dir/$lib"
  generic_cmake "-DCMAKE_POLICY_VERSION_MINIMUM=3.5"
  disable_nonessential "$src_dir/$lib"
  do_make_and_make_install
  change_dir "$src_dir"
}
...
build_libmp3lame() {
  ...
  change_dir "$src_dir/$lib"
  touch "no.autoreconf"
  apply_patch "$PATCHDIR/lame-3.100-emscripten.patch"
  generic_configure "--disable-nasm --disable-decoder --disable-frontend"
  disable_nonessential "$src_dir/$lib"
  do_make_and_make_install
...
build_libmysofa() {
  ...
  change_dir "$src_dir/$lib"
  gsed -i \
        's/defined(__illumos__)/defined(__illumos__) || defined(__EMSCRIPTEN__)/' \
        "$src_dir/$lib/src/hrtf/portable_endian.h"
  change_dir "$src_dir/$lib/build" 1
  local cmake_params="-DCMAKE_BUILD_TYPE=Release \
-DCMAKE_POLICY_VERSION_MINIMUM=3.10 \
-DBUILD_TESTS=OFF \
...
build_liboapv() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  apply_patch "$PATCHDIR/openapv-0.2.1.3-emscripten.patch"
  change_dir "$src_dir/$lib/build" 1
  local cmake_params="-DCMAKE_BUILD_TYPE=Release \
...
build_libopencv() {
  ...
  change_dir "$src_dir/$lib/build" 1
  local original_ldflags="$LDFLAGS"
  # Native GCC option; don't pass it to Emscripten.
  export LDFLAGS="${LDFLAGS//-static-libstdc++/}"
  local cmake_params="-DCMAKE_BUILD_TYPE=Release \
-DBUILD_SHARED_LIBS=OFF \
-DENABLE_PIC=OFF \
...
-DOPENCV_GENERATE_PKGCONFIG=ON"
  ...
  if [[ -f "$install_pkgconfig_dir/opencv4.pc" ]]; then
    copy_path \
      "$install_pkgconfig_dir/opencv4.pc" \
      "$install_pkgconfig_dir/opencv.pc" \
      -f
  elif [[ -f "$src_dir/$lib/build/unix-install/opencv4.pc" ]]; then
    copy_path \
      "$src_dir/$lib/build/unix-install/opencv4.pc" \
      "$install_pkgconfig_dir/opencv.pc" \
      -f
  fi
  export LDFLAGS="$original_ldflags"
...
build_libopenh264() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  apply_patch "$PATCHDIR/openh264-2.6.0-emscripten.patch"
  local meson_options="-Dtests=disabled"
  generic_meson "$meson_options"
  do_ninja_and_ninja_install
  change_dir "$src_dir"
...
build_libunwind() {
  ...
  python3 "$emscripten_root/embuilder.py" \
      build libunwind-wasmexcept > >(redirect_output) 2>&1
  # local lib="libunwind"
...
-Dcpp_link_args=\"-static -lpthread\" $config_options" # https://mesonbuild.com/Dependencies.html#shaderc trigger use of shaderc_combined
  generic_meson "$meson_options"
  # disable_nonessential "$src_dir/$lib"
  do_ninja_and_ninja_install
...
build_librist() {
  ...
  export CXXFLAGS="$CXXFLAGS -D_GNU_SOURCE -pthread"
  export CPPFLAGS="$CPPFLAGS -D_GNU_SOURCE"
  export LDFLAGS="$LDFLAGS -pthread"
  apply_patch "$PATCHDIR/librist-0.2.11-emscripten-endian-shim.patch"
  apply_patch "$PATCHDIR/librist-0.2.11-emscripten-network.patch"
  local meson_options="-Duse_mbedtls=true \
-Dc_args=\"-D_GNU_SOURCE -pthread\" \
-Dbuilt_tools=false \
-Dtest=false \
-Duse_tun=false \
...
build_libshaderc() {
  ...
  change_dir "$src_dir/$lib/build" 1
  export CFLAGS="$CFLAGS -pthread -s USE_PTHREADS=1"
  export CXXFLAGS="$CXXFLAGS -pthread -s USE_PTHREADS=1"
  export CPPFLAGS="$CPPFLAGS -pthread -s USE_PTHREADS=1"
  do_cmake_from_build_dir "$src_dir/$lib" "-DCMAKE_BUILD_TYPE=release \
...
build_libsnappy() {
  ...
  change_dir "$src_dir/$lib"
  local original_cxxflags="$CXXFLAGS"
  export CXXFLAGS="$CXXFLAGS -Wno-error=pass-failed"
  generic_cmake "-DCMAKE_BUILD_TYPE=Release \
-DBUILD_SHARED_LIBS=OFF \
...
-DSNAPPY_REQUIRE_AVX2=OFF" "$src_dir/$lib"
  disable_nonessential "$src_dir/$lib"
  do_make_and_make_install
  export CXXFLAGS="$original_cxxflags"
  change_dir "$src_dir"
...
build_libarchive() {
  ...
  export CFLAGS="$CFLAGS -I${dependency_install_prefix}/include -pthread -DUSE_PTHREAD=1"
  export LDFLAGS="$LDFLAGS -L${dependency_install_prefix}/lib -L${dependency_install_prefix}/lib/${host_target} "
  generic_configure "--enable-static \
--disable-shared \
...
-Dbuilt_in_models=true"
  disable_nonessential "$src_dir/$lib/libvmaf"
  generic_meson "$meson_options"
  do_ninja_and_ninja_install
...
build_libvpx() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  do_configure "--target=generic-gnu \
--prefix=$dependency_install_prefix \
--enable-static \
--disable-shared \
--disable-examples \
...
build_libx264() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  apply_patch "$PATCHDIR/x264-stable-emscripten.patch"
...
build_libx265() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  apply_patch "$PATCHDIR/x265-4.1-emscripten.patch"
...
build_libxavs() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  apply_patch "$PATCHDIR/xavs-distrotech-emscripten.patch"
  apply_patch "$PATCHDIR/xavs-distrotech-wasm-arch.patch"
  apply_patch "$PATCHDIR/xavs-distrotech-wasm-flags.patch"
  do_configure "--host=$host_target \
--prefix=$dependency_install_prefix \
--bindir=$dependency_install_prefix/bin \
--libdir=$dependency_install_prefix/lib \
--disable-pthread --disable-asm --enable-pic \
--extra-cflags=\"$CFLAGS\" --extra-ldflags=\"$LDFLAGS\""
  disable_nonessential "$src_dir/$lib"
  generic_make "libxavs.a"
  create_dir "$dependency_install_prefix/include"
  create_dir "$dependency_install_prefix/lib/pkgconfig"
  cp -f xavs.h "$dependency_install_prefix/include/xavs.h"
  cp -f libxavs.a "$dependency_install_prefix/lib/libxavs.a"
  cp -f xavs.pc "$dependency_install_prefix/lib/pkgconfig/xavs.pc"
  "$RANLIB" "$dependency_install_prefix/lib/libxavs.a"
  change_dir "$src_dir"
}
...
build_libxavs2() {
    ...
    # The bundled config.sub is too old to recognize Emscripten.
    get_config_sub "$src_dir/$lib/build/linux" > /dev/null 2>&1
    get_config_guess "$src_dir/$lib/build/linux" > /dev/null 2>&1

    local configure_file="$src_dir/$lib/build/linux/configure"

    # xavs2's hand-written configure does not know Emscripten.
    if ! grep -q 'emscripten\*)' "$configure_file"; then
        gsed -i '/^[[:space:]]*\*linux\*)/i\
    emscripten*)\
        SYS="EMSCRIPTEN"\
    ...
    # version.sh assumes origin/master exists, which is false for the
    # shallow detached-tag checkout used by the builder.
    gsed -i \
        's/git rev-list --count origin\/master/git rev-list --count HEAD/' \
        "$src_dir/$lib/version.sh"
    ...
    # Prevent Make from invoking a bare ./configure before the real
    # cross-configure step.
    [[ -f config.mak ]] || : > config.mak
    [[ -f config.h ]] || : > config.h
...
--disable-opencl"
    ...
    # xavs2 expects AR itself to contain the "rc" operation.
    # Also preserve AS=empty from --disable-asm.
    do_make_and_make_install "AR=\"$AR rc \" AS="

    gsed -i "s/Version:.*/Version: ${repo_ver}.0/g" \
        "$dependency_install_prefix/lib/pkgconfig/xavs2.pc"
...
-DCMAKE_BUILD_TYPE=Release"
  ...
  gsed -i 's|^includedir=.*|includedir=${prefix}/include|g' "$src_dir/$lib/build/xevd.pc"
...
-DCMAKE_BUILD_TYPE=Release"
  ...
  gsed -i 's|^includedir=.*|includedir=${prefix}/include|g' "$src_dir/$lib/build/xeve.pc"
  # manually install static library only
...
-DZMQ_HAVE_PTHREAD_SET_AFFINITY=OFF \
-DZMQ_HAVE_SO_PEERCRED=OFF \
-DZMQ_HAVE_LOCAL_PEERCRED=OFF \
-DZMQ_HAVE_BUSY_POLL=OFF \
-DCMAKE_POLICY_VERSION_MINIMUM=3.5 \
-DHAVE_PSELECT=OFF"
    do_cmake_from_build_dir "$src_dir/$lib" "$cmake_params"
    # CMake assumes IPC/AF_UNIX for every non-Windows target.
    # That assumption is not appropriate for a browser-oriented Emscripten build.
    sed -i \
        -e '/^#define ZMQ_HAVE_IPC/d' \
        -e '/^#define ZMQ_HAVE_STRUCT_SOCKADDR_UN/d' \
        "$src_dir/$lib/build/platform.hpp"
    disable_nonessential "$src_dir/$lib/build"
    do_make_and_make_install
    change_dir "$src_dir"
}
...
build_opencl() {
  echo "INFO: Skipping OpenCL: browsers do not provide an OpenCL runtime or ICD vendor drivers." | tee -a "$LOG_FILE"
}
build_glew() {
  local lib="glew"
...
build_glfw() {
  ...
  export CPPFLAGS="$CPPFLAGS -D_POSIX_C_SOURCE=200809L"
...
build_openssl() {
  ...
  touch "no.autoreconf"
  apply_patch "$PATCHDIR/openssl-3.6.0-wasi-config.patch"
  do_configure "$host_name \
--release \
--prefix=$dependency_install_prefix \
...
build_gstreamer() {
  ...
  change_dir "$src_dir"
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib/subprojects/gstreamer"
  local gst_config="$src_dir/$lib/subprojects/gstreamer/gst/gstconfig.h.in"
  if ! grep -q 'defined(__wasm__)' "$gst_config"; then
    gsed -i \
      's/#elif defined(__i386__)/#elif defined(__wasm__) || defined(__wasm32__) || defined(__i386__)/' \
      "$gst_config"
  fi
  local meson_options="-Ddoc=disabled \
-Dexamples=disabled \
...
build_vapoursynth() {
  ...
  do_git_checkout "$repo" "$src_dir/$lib" "$repo_ver"
  change_dir "$src_dir/$lib"
  generic_meson "-Denable_vsscript=false -Denable_vspipe=false -Denable_python_module=false"
  disable_nonessential "$src_dir/$lib"
  do_ninja_and_ninja_install
  change_dir "$src_dir"
  reset_allflags

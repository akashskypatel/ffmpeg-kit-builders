...
activate_emsdk() {
  local emsdk_root="$EMSDK_ROOT"
  local bootstrap_python="$EMSDK_BOOTSTRAP_PYTHON"

  export EMSDK_QUIET=1

  # emsdk_env.sh may clear caller-defined EMSDK_* variables.
  ...
  # Modern emsdk does not export EM_CONFIG itself.
...
EOF
  ...
  if [[ -n "${GITHUB_PATH:-}" ]]; then
    for path in "$emcc_dir" "$node_dir" "$binaryen_dir"; do
      grep -qxF "$path" "$GITHUB_PATH" 2>/dev/null ||

...
activate_emsdk() {
  ...
  # emsdk_env.sh configures the active emsdk PATH/tool environment.
  # Modern emsdk versions do not export EM_CONFIG themselves.
  # shellcheck source=/dev/null
  source "${EMSDK_ROOT}/emsdk_env.sh" >/dev/null

  export EMSDK_BOOTSTRAP_PYTHON="$bootstrap_python"

  # Pin Emscripten to the configuration generated by `emsdk activate`.
...
# emsdk 4.x does not export EM_CONFIG itself.
  ...
  if [[ -n "${GITHUB_ENV:-}" ]]; then
    {
      ...
      # EM_CACHE is no longer exported by modern emsdk versions.
      # Preserve it only when the caller explicitly configured one.
      if [[ -n "${EM_CACHE:-}" ]]; then
